# TRIZ_CORE_INTEGRATION_CONTRACT_v0.2

CHANGE NOTE: добавлено требование структурированного fail_reasons и запрет на парсинг текстовых строк. Уточнено потребление fail_reasons Guided‑слоями.
...
- `gate_results.GateTRIZ.fail_reasons` — **массив объектов**:
  `{ code: string, msg: string, path: string }`
- Запрещено агрегировать причины в одну строку; текст допускается только в `msg` (1 строка).
...
- fail_reasons отслеживаемы по code/path, не по текстовому парсингу.
- Guided‑слои должны потреблять причины **по code/path**, а не по строкам.
